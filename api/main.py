...
def get_pregnancy_data(language):
    if language and language != "english":
        try:
            mod = __import__(f"pregnancy_data_{language}", fromlist=[f"pregnancy_data_{language}"])
            return getattr(mod, f"pregnancy_data_{language}")
        except Exception:
            logging.info(f"No pregnancy_data_{language} module found, falling back to English")
...
def get_cervical_data(language):
    if language and language != "english":
        try:
            mod = __import__(f"cervical_cancer_data_{language}", fromlist=[f"cervical_cancer_data_{language}"])
            return getattr(mod, f"cervical_cancer_data_{language}")
        except Exception:
            logging.info(f"No cervical_cancer_data_{language} module found, falling back to English")
